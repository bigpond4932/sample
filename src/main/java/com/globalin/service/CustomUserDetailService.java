...
// 이 클래스는 스프링 시큐리티의 유저 정보를 (인증,권한) DB에서 가져와 주는 역할을 하는 객체
public class CustomUserDetailService implements UserDetailsService{
	...
	/*
	 * UserDetailsService 는 loadUserByUserName() 이라는 하나의 추상 메소드만 갖는다. 
	 * 리턴타입이 org.springframework.core.userdatails.UserDetails
	 * 스프링에서 취급하는 사용자 정보 객체는 오직 userDetails타입
	 * 우리가 만든 사용자 정보객체는 MemberVO타입이다.
	 * 그래서 우리가 만든 사용자 정보객체를 스프링 시큐리티가 사용할 수 있도록
	 * MemberVO 타입을 UserDetails 타입으로 변환하는 작업이 피룡하다.
	 * 
	 * 방법은 두 가지가 있는데
	 * 1) MemberVO 클래스를 UserDetails 인터페이스를 구현하도록 하는 방법
	 * 2) 제 3의 클래스를 이용해서 MemberVO 클래스를 수정하지 않고 하는 방버
	 */
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		// 메퍼가 vo를 가져왔으면 MemberVO를 스프링 시키류티가 사용하는 타입으로 변경
